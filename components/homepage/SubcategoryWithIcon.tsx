...
const SubcategoryWithIconLink: React.FC<SubcategoryWithIconProps> = ({ embeddable, text, onClick, url }) => {
    ...
    return (
        <div onClick={() => onClick(text)} className={`flex items-center gap-2 py-4 text-center w-full grow hover:bg-[#EBFD01] cursor-pointer px-6 last:mb-[100px] ${componentContext.selectedSubCategory == text ? 'bg-[#EBFD01]' : ''}`}>
            { !error ?
                <img onError={() => setError(true)} className="h-4 w-4 rounded-full" src={!embeddable ? './default-icon.png' : getFaviconFromLink(url)||""} alt="" />
                :
